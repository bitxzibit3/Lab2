...
bool where_foo (char sign, complex number, complex value) //TODO Тоже блять нихуя не работает, надо че то перепилить
{
    if (sign == '=')
        return value == number;
    else
...
int dialog(int seq)
{
    ...
    switch (choise)
    {
        ...
        case(6):
        {
            ...
            Polynom<T> *ans = new_Polynom(after_where, seq);
            ans->Print();
            SavePolynom(lib, first);
            SavePolynom(lib, ans);
            delete after_where;
